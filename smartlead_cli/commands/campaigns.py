...
from smartlead_cli.output import campaign_leads_table, campaigns_table, emit, print_error, statistics_table
...
analytics_app = typer.Typer(name="analytics", help="Campaign analytics helpers.", no_args_is_help=True)
...
def campaigns_list(
    ...
    data = to_data(api_request(cfg, "GET", "/campaigns", params=params))
...
def campaigns_get(
    ...
    cfg: Config = ctx.obj
    data = to_data(api_request(cfg, "GET", f"/campaigns/{campaign_id}"))
...
def campaigns_create(
    ctx: typer.Context,
    name: str | None = typer.Option(None, "--name", help="Campaign name (required unless body provided)"),
    client_id: int | None = typer.Option(None, "--client-id", help="Client ID (optional)"),
    body_json: str | None = typer.Option(None, "--body-json", help="Raw request JSON or '-' for stdin"),
    body_file: str | None = typer.Option(None, "--body-file", help="Path to JSON request body or '-' for stdin"),
) -> None:
    cfg: Config = ctx.obj
    try:
    ...
    if body is None:
        if not name:
            print_error("validation_error", "Provide --name or a request body via --body-json/--body-file")
            raise typer.Exit(1)
        body = {"name": name}
        if client_id is not None:
            body["client_id"] = client_id

    data = to_data(api_request(cfg, "POST", "/campaigns/create", json_body=body))
    emit(data, pretty=cfg.pretty)
...
def campaigns_delete(
    ...
    if not yes:
        ...
        raise typer.Exit(1)
    cfg: Config = ctx.obj
    data = to_data(api_request(cfg, "DELETE", f"/campaigns/{campaign_id}"))
    emit(data, pretty=cfg.pretty)
...
def campaigns_status(
    ctx: typer.Context,
    campaign_id: int = typer.Argument(..., help="Campaign ID"),
    status: str = typer.Option(..., "--status", help="Campaign status (e.g. START, PAUSED, STOPPED)"),
) -> None:
    cfg: Config = ctx.obj
    data = to_data(api_request(cfg, "POST", f"/campaigns/{campaign_id}/status", params={"status": status}))
    emit(data, pretty=cfg.pretty)
...
def campaigns_statistics(
    ...
    offset: int | None = typer.Option(None, "--offset", help="Pagination offset"),
    limit: int | None = typer.Option(None, "--limit", help="Pagination limit (defaults to config)"),
    email_sequence_number: int | None = typer.Option(None, "--email-sequence-number", help="Sequence step filter"),
    ...
    data = to_data(api_request(cfg, "GET", f"/campaigns/{campaign_id}/statistics", params=params))
...
def analytics_top(
    ...
    cfg: Config = ctx.obj
    data = to_data(api_request(cfg, "GET", f"/campaigns/{campaign_id}/analytics"))
    emit(data, pretty=cfg.pretty)
...
def campaign_leads_list(
    ...
    cfg: Config = ctx.obj
    params = {"offset": offset, "limit": limit if limit is not None else cfg.default_limit}
    data = to_data(api_request(cfg, "GET", f"/campaigns/{campaign_id}/leads", params=params))
...
def campaign_leads_add(
    ...
    body_file: str | None = typer.Option(None, "--body-file", help="Path to JSON payload or '-' for stdin"),
) -> None:
    cfg: Config = ctx.obj
    try:
        ...
        print_error("validation_error", f"Invalid request body: {exc}")
        raise typer.Exit(1)
    data = to_data(api_request(cfg, "POST", f"/campaigns/{campaign_id}/leads", json_body=body))
    emit(data, pretty=cfg.pretty)
...
def campaign_leads_update(
    ...
    try:
        ...
        print_error("validation_error", f"Invalid request body: {exc}")
        raise typer.Exit(1)
    data = to_data(api_request(cfg, "PATCH", f"/campaigns/{campaign_id}/leads/{lead_id}", json_body=body))
    emit(data, pretty=cfg.pretty)
...
def campaign_leads_pause(
    ...
    cfg: Config = ctx.obj
    data = to_data(api_request(cfg, "POST", f"/campaigns/{campaign_id}/leads/{lead_id}/pause"))
    emit(data, pretty=cfg.pretty)
...
def campaign_leads_resume(
    ...
    campaign_id: int = typer.Argument(..., help="Campaign ID"),
    lead_id: int = typer.Argument(..., help="Lead ID"),
    delay_days: int | None = typer.Option(None, "--delay-days", min=0, help="Resume after N days (query param)"),
) -> None:
    cfg: Config = ctx.obj
    params = {"delay_in_days": delay_days} if delay_days is not None else None
    data = to_data(api_request(cfg, "POST", f"/campaigns/{campaign_id}/leads/{lead_id}/resume", params=params))
    emit(data, pretty=cfg.pretty)
...
def campaign_leads_unsubscribe(
    ...
    cfg: Config = ctx.obj
    data = to_data(api_request(cfg, "POST", f"/campaigns/{campaign_id}/leads/{lead_id}/unsubscribe"))
    emit(data, pretty=cfg.pretty)
...
def campaign_leads_delete(
    ...
    if not yes:
        ...
        raise typer.Exit(1)
    cfg: Config = ctx.obj
    data = to_data(api_request(cfg, "DELETE", f"/campaigns/{campaign_id}/leads/{lead_id}"))
    emit(data, pretty=cfg.pretty)
...
def campaign_leads_message_history(
    ...
    cfg: Config = ctx.obj
    data = to_data(api_request(cfg, "GET", f"/campaigns/{campaign_id}/leads/{lead_id}/message-history"))
    emit(data, pretty=cfg.pretty)
